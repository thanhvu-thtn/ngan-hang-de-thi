...
@section('title', 'Thêm Loại chuyên đề mới')

@section('content')
<div class="max-w-3xl mx-auto">
    <div class="mb-6 flex items-center gap-3">
        <a href="{{ route('topic-types.index') }}" class="w-10 h-10 bg-slate-100 text-slate-600 hover:bg-slate-200 hover:text-blue-600 rounded-full flex items-center justify-center transition shadow-sm border border-slate-200">
            <i class="fa-solid fa-arrow-left"></i>
        </a>
        <div>
            <h2 class="text-2xl font-bold text-slate-800">Thêm Loại chuyên đề mới</h2>
            <p class="text-slate-500 text-sm mt-1">Nhập thông tin cho loại chuyên đề mới</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <form action="{{ route('topic-types.store') }}" method="POST">
            ...
            <div class="bg-slate-50 px-6 py-4 border-t border-slate-200 flex justify-end gap-4">
                ...
                <button type="submit" class="inline-flex items-center justify-center gap-2 px-8 py-2.5 text-sm font-medium bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition shadow-sm">
                    <i class="fa-solid fa-floppy-disk"></i> Lưu loại chuyên đề
                </button>
            </div>
        </form>
    </div>
