...
<div class="container mx-auto px-4 py-6 max-w-3xl">
    <div class="mb-6 flex items-center justify-between">
        <h2 class="text-2xl font-bold text-gray-800">Chỉnh sửa Nội dung</h2>
        <a href="{{ route('contents.index') }}" class="text-indigo-600 hover:text-indigo-800 font-medium">
            <i class="fa-solid fa-arrow-left mr-1"></i> Quay lại
        </a>
    </div>

    <div class="bg-white rounded-lg shadow-md p-6 border border-gray-100">
        <form action="{{ route('contents.update', ['content' => $content->id] + request()->query()) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-5">
                ...
                <input type="text" name="name" value="{{ old('name', $content->name) }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('name') border-red-500 @enderror" required>
            ...
            <div class="grid grid-cols-2 gap-5 mb-6">
                <div>
                    ...
                    <input type="number" name="periods" min="0" value="{{ old('periods', $content->periods) }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('periods') border-red-500 @enderror" required>
                ...
                <div>
                    ...
                    <input type="number" name="order" value="{{ old('order', $content->order) }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('order') border-red-500 @enderror">
            ...
            <div class="flex justify-end pt-4 border-t border-gray-100">
                <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-6 rounded-md shadow-sm transition duration-150">
                    <i class="fa-solid fa-floppy-disk mr-2"></i> Cập nhật Nội dung
                </button>
            </div>
        </form>
    </div>
