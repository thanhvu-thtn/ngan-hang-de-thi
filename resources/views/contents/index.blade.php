...
@section('content')
    <div class="flex justify-between items-center mb-6">
        <div class="flex items-center space-x-4">
            <a href="{{ request('back_url') ? urldecode(request('back_url')) : route('topics.index') }}"
        ...
        <a href="{{ route('contents.create', array_merge(request()->only(['topic_id']), ['back_url' => urlencode(request()->fullUrl())])) }}"
    ...
    <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-100 mb-6">
            ...
            <div class="lg:col-span-3">
                <select name="topic_type_id" id="filterTopicType"
                    ...
                    @foreach ($topicTypes as $type)
                        <option value="{{ $type->id }}" {{ request('topic_type_id') == $type->id ? 'selected' : '' }}>
                            {{ $type->name }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <form action="{{ route('contents.index') }}" method="GET"
            ...
            <div class="lg:col-span-2 flex gap-2">
                ...
                <a href="{{ route('contents.index', request()->has('back_url') ? ['back_url' => request('back_url')] : []) }}"
                    class="w-full bg-gray-200 hover:bg-gray-300 text-gray-700 text-sm font-medium py-2 px-3 rounded-md shadow-sm transition duration-150 flex items-center justify-center text-center">
                    Xóa
                </a>
            </div>
                        ...
                        <tr class="border-b border-gray-100 hover:bg-slate-50 transition duration-150">
                            ...
                            <td class="py-3 px-6 text-center">
                                <div class="flex item-center justify-center space-x-3">
                                    ...
                                    <a href="{{ route('contents.edit', ['content' => $content->id, 'back_url' => urlencode(request()->fullUrl())]) }}"
