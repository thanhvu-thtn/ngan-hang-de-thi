    ...
    <div class="p-4">
        ...
        <div id="menu-cau-hoi" class="sidebar-group mt-6">
            ...
            <ul class="space-y-1">
                ...
                <li><a href="#" class="block px-3 py-2 text-sm font-medium text-slate-700 rounded-md hover:bg-slate-100 hover:text-slate-900 transition"><i class="fa-solid fa-edit w-5 text-center mr-2 text-slate-400"></i> Biên tập câu hỏi</a></li>
                @endcan
            </ul>
        </div>
        @endcanany
        ...
        <div id="menu-de-thi" class="sidebar-group mt-6">
            ...
            <ul class="space-y-1">
                <li><a href="#" class="block px-3 py-2 text-sm font-medium text-slate-700 rounded-md hover:bg-slate-100 hover:text-slate-900 transition"><i class="fa-solid fa-table w-5 text-center mr-2 text-slate-400"></i> Xây dựng đề thi</a></li>
            </ul>
        </div>
        @endcan
    ...
    <div class="absolute bottom-0 left-0 w-64 p-4 border-t border-slate-200 bg-white">
        <div class="flex items-center gap-3 mb-4 px-2">
            ...
            <div class="overflow-hidden">
                ...
                <p class="text-xs text-slate-500 truncate">{{ auth()->user()->roles->pluck('name')->first() ?? 'Chưa cấp quyền' }}</p>
            </div>
        </div>
        
        <form action="{{ route('logout') }}" method="POST">
            @csrf
            <button type="submit" class="w-full flex items-center gap-3 px-4 py-2.5 text-rose-600 rounded-lg hover:bg-rose-50 transition font-medium">
