    ...
    <div class="space-y-6">
        ...
        <div>
            ...
            <div class="border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                <textarea id="editor-stem" name="stem" class="w-full border-0 focus:ring-0" rows="4">{!! old('stem') !!}</textarea>
            </div>
        </div>
        ...
        <div>
            <div class="flex justify-between items-end mb-3">
                <label class="block text-sm font-semibold text-slate-700">Các phương án lựa chọn (Mặc định 4 phương án)
                    <span class="text-rose-500">*</span></label>
            </div>

            <div class="space-y-5">
                {{-- Lặp 4 lần để tạo 4 ô đáp án trống --}}
                    ...
                    <div
                        ...
                        <div class="border border-slate-200 rounded-xl overflow-hidden shadow-sm mb-4 bg-white">
                            <textarea id="editor-choice-{{ $index }}" name="choices[{{ $index }}][content]"
                                class="w-full border-0 focus:ring-0 editor-choice" rows="3">{!! old("choices.{$index}.content") !!}</textarea>
                        </div>
                        ...
                        <div
                            ...
                            <label class="flex items-center gap-2.5 cursor-pointer group">
                                <input type="radio" name="correct_choice" value="{{ $index }}"
                                    {{ old('correct_choice') == $index ? 'checked' : '' }}
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <input type="number" id="ratio-{{ $index }}"
                                    ...
                                    value="{{ old("choices.{$index}.ratio", 1) }}"
        ...
        {{-- KHUNG 3: CẤU HÌNH HIỂN THỊ (LAYOUT) --}}
        <div>
            ...
            {{-- Lấy ID của layout cuối cùng làm mặc định (nếu có dữ liệu) --}}
            @php
            ...
            <select id="layout_id" name="layout_id"
                ...
                @isset($layouts)
                    @foreach ($layouts as $layout)
                        {{-- Hàm old sẽ lấy $defaultLayoutId nếu user chưa chọn gì trước đó --}}
        ...
        <div>
            ...
            <div class="border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                ...
                <textarea id="editor-explanation" name="explanation" class="w-full border-0 focus:ring-0 p-3" rows="4">{!! old('explanation') !!}</textarea>
            </div>
        </div>
    </div>
</div>
