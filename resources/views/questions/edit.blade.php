...
<div class="container mx-auto p-6 max-w-5xl">
    ...
    <form action="{{ route('questions.update', $question->id) }}" method="POST" id="question-edit-form">
        ...
        <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Tóm tắt nội dung (Name) <span class="text-rose-500">*</span></label>
                    <input type="text" name="name" value="{{ old('name', $question->name) }}" class="w-full border-slate-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 shadow-sm">
                    @error('name') <p class="text-sm text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>

                {{-- Mã định danh --}}
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Mã định danh (Tag Name) <span class="text-rose-500">*</span></label>
                    <input type="text" id="tag_name_input" name="tag_name" value="{{ old('tag_name', $question->tag_name) }}" class="w-full border-slate-300 rounded-xl shadow-sm">
                    <p id="tag_name_warning" class="text-sm text-rose-600 mt-1 hidden font-medium">Mã đã tồn tại!</p>
                </div>

                {{-- Mức độ nhận thức --}}
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Mức độ nhận thức <span class="text-rose-500">*</span></label>
                    <select name="cognitive_level_id" class="w-full border-slate-300 rounded-xl shadow-sm">
                        @foreach (\App\Models\CognitiveLevel::all() as $level)
                            <option value="{{ $level->id }}" {{ old('cognitive_level_id', $question->cognitive_level_id) == $level->id ? 'selected' : '' }}>{{ $level->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    @can('tham-dinh-cau-hoi')
                        <select name="status" class="w-full border-amber-300 bg-amber-50 rounded-xl focus:ring-amber-500 shadow-sm">
                            <option value="0" {{ old('status', $question->status) == 0 ? 'selected' : '' }}>Chờ thẩm định</option>
                            <option value="1" {{ old('status', $question->status) == 1 ? 'selected' : '' }}>Đã duyệt</option>
                            <option value="2" {{ old('status', $question->status) == 2 ? 'selected' : '' }}>Cần sửa lại</option>
                        </select>
                    @else
                        <div class="py-2.5 px-4 bg-slate-100 border border-slate-200 rounded-xl text-slate-600 flex items-center gap-2">
                ...
                <div>
                    ...
                    @can('tham-dinh-cau-hoi')
                        <input type="number" step="0.1" name="difficulty_index" value="{{ old('difficulty_index', $question->difficulty_index) }}" class="w-full border-amber-300 bg-amber-50 rounded-xl shadow-sm">
                    @else
        ...
        <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 mb-6">
            <h3 class="text-lg font-bold text-slate-800 mb-6 border-b pb-2 flex items-center gap-2">
                <i class="fa-solid fa-crosshairs text-rose-500"></i> Mục tiêu đánh giá <span class="text-rose-500">*</span>
            </h3>
            ...
            @error('objective_ids') <p class="text-sm text-red-500 mt-2">{{ $message }}</p> @enderror
        </div>
        ...
        <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 mb-6">
            <h3 class="text-lg font-bold text-slate-800 mb-6 border-b pb-2 flex items-center gap-2">
                <i class="fa-solid fa-file-lines text-emerald-500"></i> Nội dung chi tiết
            </h3>
        ...
        <div class="mt-8 flex justify-end gap-3 sticky bottom-4">
            <a href="{{ route('questions.index') }}" class="px-6 py-2.5 bg-white border border-slate-300 text-slate-700 rounded-xl hover:bg-slate-50 font-semibold transition">Hủy bỏ</a>
            <button type="submit" class="px-6 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 font-bold transition shadow-lg shadow-blue-200 flex items-center gap-2">
...
@push('scripts')
    @include('partials.editor_script', ['selector' => '#editor-stem, #editor-explanation'])
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            tagInput.addEventListener('blur', function() {
                ...
                fetch(`/questions/check-tag-name?tag_name=${encodeURIComponent(val)}`)
                    .then(r => r.json())
                    .then(data => {
                        data.exists ? warningMsg.classList.remove('hidden') : warningMsg.classList.add('hidden');
                    });
            });
        });
    </script>
