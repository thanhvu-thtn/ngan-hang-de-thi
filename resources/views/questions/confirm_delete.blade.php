    ...
    <div class="container mx-auto px-4 py-6 md:px-6 lg:px-8 max-w-5xl">
        ...
        <div class="mb-6 p-4 bg-rose-50 border border-rose-200 rounded-xl flex items-start gap-3">
            ...
            <div>
                ...
                <p class="text-rose-600 text-sm">Bạn đang yêu cầu xóa vĩnh viễn câu hỏi này. <strong>Không thể hoàn tác!</strong></p>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between pb-4 border-b border-gray-200 mb-6">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">
                    @if (!empty($question->shared_context_id))
                        <a href="#" class="text-indigo-600 hover:text-indigo-800 hover:underline transition-colors">Câu hỏi có dữ liệu dùng chung</a> |
                    @endif
                    Loại: {{ $question->questionType->name ?? 'N/A' }} | Ngày tạo: {{ $question->created_at->format('d/m/Y') }}
                </p>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                {{-- 2. Nút Xóa --}}
                <form action="{{ route('questions.destroy', $question->id) }}" method="POST" class="inline-block m-0" onsubmit="return confirm('Bạn có chắc chắn muốn xóa vĩnh viễn câu hỏi này không?');">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            @if ($question->status == 1)
                                <span class="px-2 py-1 bg-green-100 text-green-700 rounded-md text-xs font-medium border border-green-200"><i class="fa-solid fa-check-circle mr-1"></i> Đã thẩm định</span>
                            @else
                                <span class="px-2 py-1 bg-amber-100 text-amber-700 rounded-md text-xs font-medium border border-amber-200"><i class="fa-solid fa-clock mr-1"></i> Chờ thẩm định</span>
                            @endif
                        </div>
                        <div>
                            <span class="block text-xs text-gray-500 mb-1">Mức độ nhận thức</span>
                            <span class="text-sm font-medium text-gray-800">{{ $question->cognitiveLevel->name ?? 'Chưa xác định' }}</span>
                        </div>
                        <div>
                            <span class="block text-xs text-gray-500 mb-1">Độ khó (tự đánh giá)</span>
                            <span class="text-sm font-medium text-gray-800">{{ $question->difficulty_index ?? 'Chưa đánh giá' }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-span-1">
                <div class="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100 shadow-sm h-full">
                    <h3 class="text-xs font-bold text-indigo-400 uppercase tracking-wider mb-3">Mục tiêu đánh giá (YCCĐ)</h3>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-8">
                        ...
                        <div class="space-y-3">
                            @foreach ($question->choices as $index => $choice)
                                @php
                                    ...
                                    $bgColor = $isCorrect ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200';
                                    ...
                                    $choiceContent = $choice->choice_text ?? $choice->content ?? $choice->text ?? $choice->description ?? '';
                                @endphp
                                <div class="flex items-start p-3 rounded-lg border {{ $bgColor }}">
                                    <div class="flex-shrink-0 mt-0.5 mr-3">
                                        <span class="flex items-center justify-center w-6 h-6 rounded-full border {{ $isCorrect ? 'border-green-500 bg-green-100 text-green-700' : 'border-gray-300 bg-white text-gray-500' }} text-xs font-bold">
                                            {{ $label }}
                                        </span>
                                    </div>
            ...
            @if ($question->explanation)
                <div class="bg-gray-50 border-t border-gray-200 p-6">
                    ...
                    <div class="prose max-w-none text-gray-700 text-sm format-katex bg-white p-4 rounded-lg border border-gray-200">
