...
@section('title', 'Cập nhật Chuyên đề')
...
<div class="max-w-4xl mx-auto">
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            <h2 class="text-2xl font-bold text-slate-800">Cập nhật Chuyên đề</h2>
            <p class="text-slate-500 text-sm mt-1">Chỉnh sửa thông tin của chuyên đề: <span class="font-semibold text-blue-600">{{ $topic->name }}</span></p>
        </div>
    </div>
        ...
        <form action="{{ route('topics.update', $topic->id) }}" method="POST" class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div>
                    ...
                    <input type="number" name="order" id="order" value="{{ old('order', $topic->order) }}" min="1" required
                        class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:bg-white outline-none transition-all">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="total_periods" id="total_periods" value="{{ old('total_periods', $topic->total_periods) }}" min="1" required
                        class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:bg-white outline-none transition-all">
                </div>
            </div>
