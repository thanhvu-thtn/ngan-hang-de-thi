        ...
        <div class="mb-6 flex justify-between items-center">
            <div>
                <h2 class="text-2xl font-bold text-slate-800 flex items-center gap-3">
                    ...
                    Thêm dữ liệu dùng chung mới
                </h2>
                <p class="text-slate-500 text-sm mt-1">Dùng cho các cụm câu hỏi (đọc hiểu, nghị luận...).</p>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="tag_name" id="tag_name" value="{{ old('tag_name') }}" required
                        placeholder="Ví dụ: VB_DOC_HIEU_01"
                ...
                <div class="mb-6">
                    ...
                    <textarea name="content" id="editor-content" rows="10"
                        class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500 transition-all">{{ old('content') }}</textarea>
                    @error('content')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="note" id="note" value="{{ old('note') }}"
                        placeholder="Ghi chú nguồn văn bản, tác giả..."
                        class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500 transition-all shadow-sm">
                </div>
...
@push('scripts')
    {{-- Nếu bác dùng trình soạn thảo, gắn ID #editor-content vào đây --}}
